...
import transformers
from quantize.structure_arb import structural_guassian_distribution_multip_alternating_group_x
...
'''
TRAGPTQ is the meaning of GPTQ used Ternary Residual Approximation in paper to realize 1.58-bit quantization
TRAGPTQ uses structural mask to distinguish outliers and other data, and takes advantage of part of GPTQ to lower error
'''
class TRAGPTQ:
    def __init__(
        self, layer, braq_quantizer,salient_metric, disable_gptq=False, order2_group=False
    ):
    ...
    def fasterquant(self,
        ...
        for blocki, col_st in enumerate(range(0, self.columns, blocksize)):
            ...
            if self.order2_group:
                num_mask = 2 * (num_p+1)
                orders = [2]*(num_p+1) + [1]*(num_p+1)
            else:
                num_mask = 1 + num_p + 1
                orders = [2] + [1]*(num_p+1)
            mask = torch.zeros_like(W[:, st:ed], dtype=torch.bool).unsqueeze(0).repeat_interleave(num_mask, dim=0)
            mask_list = structural_guassian_distribution_multip_alternating_group_x(
                W[:, st:ed], H[st:ed, st:ed], self.salient_metric, 
                64, num_p, S, self.order2_group
            )
            ...
            if self.disable_gptq:
                ...
                q_part_groups = [self.braq_quantizer.quantize(W1, mask[i], order=orders[i], S=S) for i in range(mask.shape[0])]
        ...
        if isinstance(self.layer, transformers.Conv1D):
            Q = Q.t()
        #torch.save(W.detach().cpu().float(), "W_arb.pt")
        self.layer.weight.data = Q.contiguous().reshape(self.layer.weight.shape).to(
        ...
        # del H, Hinv, self.inp2, S, q_part_groups
